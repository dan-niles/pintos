...
/* Holds threads that are put to sleep */
...
bool cmp_wakeup_time(const struct list_elem *a,
...
{
  /* Defualt Implementation. Uses busy waiting. */
  ...
  /* Optimized Implementation. Uses a sleep list. */
  ...
  list_insert_ordered(&sleep_list, &t->elem, (list_less_func *)cmp_wakeup_time, NULL);
  thread_block(); // Blocks current thread for ticks
...
timer_interrupt(struct intr_frame *args UNUSED)
{
  ...
  /* Check if any thread can be woken up*/
  struct thread *t;
  while (!list_empty(&sleep_list))
  {
    t = list_entry(list_front(&sleep_list), struct thread, elem);

    // Check if first thread in sleep list can be woken up
