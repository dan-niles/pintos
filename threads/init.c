...
int string_compare(char *input, char *cmd_name, int size, int input_end);
...
int pintos_init(void)
{
  ...
  else
  {
    ...
    int exit = 0;      // Used to stop taking input from user
    ...
    while (true)
    {
      ...
      while (true)
      {
        input[input_end] = (char)input_getc();
      ...
      // Displays user's name alongside their index number
      if (string_compare(input, "whoami", 6, input_end))
        printf("Dan - 200421U\n");
      // Pintos OS will shutdown and exit the qemu emulator
      else if (string_compare(input, "shutdown", 8, input_end))
      {
        exit = 1;
...
int string_compare(char *str_a, char *str_b, int str_a_len, int str_b_len)
{
